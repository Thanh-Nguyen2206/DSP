"""
=============================================================================
Phase 3: Fine-tuning F5-TTS Vietnamese for Personal Voice Cloning
...
def resolve_config_paths(config: Dict[str, Any], config_path: str) -> Dict[str, Any]:
    """
    Resolve các path tương đối trong YAML theo thư mục project.

    Điều này giúp script chạy đúng cả khi gọi từ root project:
      python scripts/train.py --config configs/train_config.yaml

    hoặc từ thư mục cha:
      python f5tts_vietnamese/scripts/train.py --config f5tts_vietnamese/configs/train_config.yaml
    """
    resolved = copy.deepcopy(config)
    config_file = Path(config_path).resolve()
    project_dir = config_file.parent
    for parent in [config_file.parent, *config_file.parents]:
        if (parent / "scripts" / "train.py").exists() and (parent / "configs").exists():
            project_dir = parent
            break

    experiment_name = resolved.get("experiment", {}).get("name")
    if experiment_name:
        resolved.setdefault("checkpoint", {})
        resolved.setdefault("logging", {})
        if resolved["checkpoint"].get("save_dir", "checkpoints") == "checkpoints":
            resolved["checkpoint"]["save_dir"] = f"checkpoints/experiments/{experiment_name}"
        if resolved["logging"].get("log_dir", "logs") == "logs":
            resolved["logging"]["log_dir"] = f"logs/experiments/{experiment_name}"
    ...
    for section, key in path_fields:
        ...
        if not path.is_absolute():
            resolved[section][key] = str(project_dir / path)

    resolved["_project_dir"] = str(project_dir)
    return resolved


def validate_training_config(config: Dict[str, Any]) -> None:
    """Fail fast cho các cấu hình dễ gây lỗi/OOM trên NVIDIA T4."""
    train_cfg = config["training"]

    if train_cfg.get("mixed_precision") == "bf16":
        raise ValueError("NVIDIA T4 không hỗ trợ BF16. Hãy dùng mixed_precision='fp16'.")
    if train_cfg.get("mixed_precision") not in {"fp16", "no"}:
        raise ValueError("mixed_precision chỉ nên là 'fp16' hoặc 'no'.")
    if int(train_cfg.get("batch_size", 1)) > 2:
        raise ValueError("batch_size > 2 dễ OOM trên T4. Hãy dùng 1-2 + gradient accumulation.")
    if int(train_cfg.get("gradient_accumulation_steps", 1)) < 1:
        raise ValueError("gradient_accumulation_steps phải >= 1.")
...
def apply_audio_augmentation(
    ...
    """
    Augmentation nhẹ cho fine-tuning giọng nói cá nhân.

    Mặc định tắt. Khi bật, chỉ dùng các biến đổi an toàn về cường độ/nhiễu/dịch
    thời gian, không đổi sample rate hay độ dài segment để tránh lệch lens.
    """
    ...
    if gain_range and len(gain_range) == 2:
        gain_db = _random_uniform(float(gain_range[0]), float(gain_range[1]))
        waveform = waveform * (10 ** (gain_db / 20.0))

    shift_ms = float(cfg.get("time_shift_ms", 0.0) or 0.0)
    if shift_ms > 0 and waveform.numel() > 1:
        max_shift = int(sample_rate * shift_ms / 1000.0)
        max_shift = min(max_shift, waveform.numel() - 1)
        if max_shift > 0:
            ...
            if shift > 0:
                ...
                shift_abs = abs(shift)
                waveform = torch.cat([waveform[shift_abs:], torch.zeros(shift_abs)])

    snr_range = cfg.get("noise_snr_db_range")
    if snr_range and len(snr_range) == 2:
        signal_power = waveform.pow(2).mean()
        if signal_power > 1e-8:
            snr_db = _random_uniform(float(snr_range[0]), float(snr_range[1]))
...
class VoiceDataset(Dataset):
    ...
    def __init__(
        ...
        for line_num, line in enumerate(all_lines, start=1):
            ...
            # maxsplit=4 giữ nguyên transcript nếu trong text có ký tự "|".
    ...
    def __getitem__(self, idx):
        ...
        waveform = apply_audio_augmentation(
            waveform,
        ...
        for text in texts:
            ...
            for char in text.lower():
                if char in vocab_char_map:
                    ...
                    pass
            if not tokens:
                # Tránh batch có text shape [B, 0] nếu transcript chỉ chứa ký tự lạ.
                tokens.append(vocab_char_map.get(" ", 0))
...
def build_model(config: Dict[str, Any], vocab_char_map: dict) -> CFM:
    ...
    dit = DiT(
        ...
        text_num_embeds=len(vocab_char_map),  # DiT internally reserves the extra unknown/padding slot.
    )
...
def download_and_load_weights(model: CFM, config: Dict[str, Any]) -> CFM:
    ...
    if ckpt_filename.endswith(".safetensors"):
        ...
        # Một số checkpoint lưu dạng wrapper dict.
        if isinstance(state_dict, dict):
            for key in ("model", "state_dict", "model_state_dict"):
                if key in state_dict:
                    logger.info(f"   Unwrapped checkpoint key: {key}")
                    state_dict = state_dict[key]
                    break
            else:
                if "ema_model_state_dict" in state_dict:
                    logger.info("   Unwrapped checkpoint key: ema_model_state_dict")
                    state_dict = {
...
def download_vocab(config: Dict[str, Any]) -> dict:
    ...
    try:
        ...
        # Repo này cung cấp vocab file trực tiếp, nên dùng custom tokenizer.
...
def save_checkpoint(
    ...
    all_ckpts = sorted(
        [
            path for path in save_path.glob("step_*")
...
def load_checkpoint(
    ...
    Returns:
        Tuple (global_step, saved_epoch). Nếu không có checkpoint trả về (0, 0).
    """
    ...
    # Load model weights
    model_state = torch.load(latest_ckpt / "model.pt", map_location="cpu", weights_only=True)
    model.load_state_dict(model_state)

    # Load optimizer state
    optim_state = torch.load(latest_ckpt / "optimizer.pt", map_location="cpu", weights_only=True)
    optimizer.load_state_dict(optim_state)

    # Load training state
    train_state = torch.load(latest_ckpt / "training_state.pt", map_location="cpu", weights_only=True)
    global_step = train_state["global_step"]
    saved_epoch = train_state.get("epoch", 0)
    logger.info(f"   Resumed at step {global_step}, epoch {saved_epoch}")
...
def train(config: Dict[str, Any]):
    ...
    logger.info("=" * 60)
    logger.info(" F5-TTS Vietnamese Fine-tuning")
    logger.info("=" * 60)
    ...
    if Path(ckpt_cfg["save_dir"]).exists():
        resume_step, _ = load_checkpoint(raw_model, optimizer, ckpt_cfg["save_dir"])
        if resume_step > 0:
    ...
    if resume_step >= total_steps:
        logger.info(
            f"✅ Checkpoint hiện tại đã đạt {resume_step}/{total_steps} steps. "
            "Không cần train thêm với cấu hình epochs hiện tại."
        )
        return

    start_epoch = resume_step // max(steps_per_epoch, 1)
    skip_batches_in_epoch = resume_step % max(steps_per_epoch, 1)
    if resume_step > 0:
        logger.info(
            f"📍 Resume plan: start_epoch={start_epoch + 1}, "
            f"skip_batches_in_epoch={skip_batches_in_epoch}"
        )
    ...
    for epoch in range(start_epoch, train_cfg["epochs"]):
        ...
        for step, batch in enumerate(progress_bar):
            # Khi resume giữa epoch, bỏ qua đúng số batch đã hoàn thành.
...
def main():
    parser = argparse.ArgumentParser(
        description="F5-TTS Vietnamese Fine-tuning Script",
