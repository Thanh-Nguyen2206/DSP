...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="F5-TTS Vietnamese - Data Preprocessing Pipeline",
...
Ví dụ:
  ...
  python scripts/data_prep.py --input_dir data/raw --min_snr 35
    ...
    if args.min_duration <= 0:
        parser.error("min_duration phải > 0")
    if args.max_duration <= args.min_duration:
        parser.error("max_duration phải > min_duration")
    if args.sample_rate <= 0:
        parser.error("sample_rate phải > 0")
    if args.min_snr < 0:
        parser.error("min_snr phải >= 0")
...
if __name__ == "__main__":
    ...
    print("=" * 60)
    print(" F5-TTS Vietnamese - Data Preprocessing Pipeline")
    print("=" * 60)
