#!/usr/bin/env bash
# =============================================================================
# F5-TTS Vietnamese Fine-tuning - Environment Setup Script
# 
...
# ========================== CẤU HÌNH ==========================
ENV_NAME="f5tts_vn"         # Tên conda environment
...
echo "=============================================="
echo " F5-TTS Vietnamese - Environment Setup"
...
if command -v conda &> /dev/null; then
    echo "    Conda detected. Chuẩn bị environment '${ENV_NAME}'..."

    eval "$(conda shell.bash hook)"

    if conda env list | awk '{print $1}' | grep -qx "${ENV_NAME}"; then
        if [ "${RESET_ENV:-0}" = "1" ]; then
            echo "    RESET_ENV=1 → xóa và tạo lại environment '${ENV_NAME}'..."
            conda deactivate 2>/dev/null || true
            conda env remove -n "${ENV_NAME}" -y
            conda create -n "${ENV_NAME}" python="${PYTHON_VERSION}" -y
        else
            echo "    Environment '${ENV_NAME}' đã tồn tại. Sử dụng lại."
            echo "    Muốn tạo sạch từ đầu: RESET_ENV=1 bash setup.sh"
        fi
    else
        echo "    Tạo environment '${ENV_NAME}'..."
        conda create -n "${ENV_NAME}" python="${PYTHON_VERSION}" -y
    fi
...
if [ "$(uname)" = "Darwin" ]; then
    echo "    macOS detected. Cài đặt PyTorch bản chuẩn (hỗ trợ MPS/CPU)..."
